...
1. Call the `subagent` tool with `read_only: true` to create a concrete implementation plan. Use the current agent's model and thinking level by default. Override either value only if the user explicitly requested an override. Tell it to inspect the repository thoroughly, use only read-only shell commands, and return a concise ordered plan without modifying files.
