...
1. Call the `subagent` tool with `read_only: true`. Choose `gpt-5.6-luna` or `gpt-5.6-sol` and a reasoning effort appropriate for the scope, complexity, and risk of the review. Give it the prompt `/review ${ARGUMENTS:-} Use only read-only inspection commands and do not modify files.` This runs the existing `/review` prompt in a fresh isolated session.
...
4. Call a new subagent with the same `/review` prompt to review the updated working tree. Re-evaluate the appropriate model and reasoning effort for the updated review task. Do not reuse a prior subagent session.
