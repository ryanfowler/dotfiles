...
1. Call the `subagent` tool with `read_only: true`. Use the current agent's model and thinking level by default. Override either value only if the user explicitly requested an override. Give it the prompt `/review ${ARGUMENTS:-} Use only read-only inspection commands and do not modify files.` This runs the existing `/review` prompt in a fresh isolated session.
